...
F_vec1 <- c(seq(0.01, 0.15, length.out = n_yrs / 2), seq(0.15, 0.15, length.out = (n_yrs / 2))) 
F_vec2 <- c(seq(0.01, 0.15, length.out = n_yrs / 2), seq(0.3, 0.03, length.out = (n_yrs / 2))) 
...
for(y in 1:n_yrs) {
  for(r in 1:n_regions) {
    for(sim in 1:n_sims) {
      Fmort[y,r,1,sim] <- F_vec2[y] 
    }
    for(s in 1:n_sexes) {
      for(f in 1:n_fish_fleets) {
        fish_sel[y,r,,s,f,1] <- 
        1 / (1 + exp(-0.3 * ((1:n_ages) - 3))) 
      }
    }
  }
}
...
rec_sexratio <- array(0.5, dim = c(n_yrs, n_regions, n_sexes, n_sims))
...
init_sigmaR <- array(0.8, dim = c(1, n_regions))
sigmaR <- array(0.8, dim = c(n_yrs, n_regions))
...
movement_matrix[1,,,,1,] <- c(0.95, 0.05)
movement_matrix[2,,,,1,] <- c(0.05, 0.95)
movement_matrix[1,,,,2,] <- c(0.95, 0.05)
movement_matrix[2,,,,2,] <- c(0.05, 0.95)
  ...
  for(r in 1:n_regions) {
    for(s in 1:n_sexes) {
      for(sf in 1:n_srv_fleets) {
        srv_sel[y,r,,s,sf,] <- 1 / (1 + exp(-0.3 * ((1:n_ages) - 4)))
      }
    }
  }
}
...
for(sim in 1:n_sims) {
  ...
  if(do_recruits_move == 0) for(r in 1:n_regions) NAA[y,r,1,,sim] <- r0[y,r,sim] * exp(ln_rec_devs[y,r,sim]) * rec_sexratio[y,r,,sim] 
  } # end y loop
    ...
    for(f in 1:n_fish_fleets) {
      for(r in 1:n_regions) {
        for(s in 1:n_sexes) {
          ...
          CAA[y,r,,s,f,sim] <- (Fmort[y,r,f,sim] * fish_sel[y,r,,s,f,sim]) / Z[y,r,,s,sim] *
                                NAA[y,r,,s,sim] * (1 - exp(-Z[y,r,,s,sim]))
        ...
        Obs_Catch[y,r,f,sim] <- True_Catch[y,r,f,sim] * exp(rnorm(1, -sigmaC^2/2, sigmaC)) # Observed Catch w/ lognormal deviations
  ...
  for(y in 1:n_yrs) {
    for(sf in 1:n_srv_fleets) {
      for(r in 1:n_regions) {
        ...
        Obs_SrvIdx[y,r,sf,sim] <- srv_q[y,r,sf,sim] * True_SrvIdx[y,r,sf,sim] * exp(rnorm(1, -sigmaSrvIdx[r,sf]^2/2, sigmaSrvIdx[r,sf])) # Observed survey index w/ lognormal deviations
  ...
  for(tag_rel in 1:n_tag_rel_events) {
    ...
    n_tags_rel <- round(Obs_SrvIdx[tag_rel_yr,,1,sim] / sum(Obs_SrvIdx[tag_rel_yr,,1,sim]) * n_tags) # Figure out relative tag distribution
    tmp_SrvAgeComps_Prob <- as.vector(Srv_IAA[tag_rel_yr, tag_rel_region, , , 1, sim]) # Use survey proportions to distribute tags
    tagged_fish <- (tmp_SrvAgeComps_Prob / sum(tmp_SrvAgeComps_Prob))  * n_tags_rel # Distribute tags across ages, sexes, for a given release event
    ...
    for(recap_yr in 1:min(max_liberty, n_yrs - tag_rel_yr + 1)) { # recapture year for a given cohort - adding a cut off for time at liberty
      ...
      # Input tagged fish into available tags for recapture and adjust initial number of tagged fish for tag induced mortality (just a mortality process)
      ...
      tmp_non_recap <- (tmp_n_tags_rel - sum(Pred_Tag_Recap[1:recap_yr,tag_rel,,,,sim])) / tmp_n_tags_rel # calculate non recapture probabilities
