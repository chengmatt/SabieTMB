...
model_name_prev<-"21.12"                                 # final model name for model used in previous year/SAFE
...
OFL_ABC_ACT_prev <- as.matrix(c(47390,	40502,	39637))  # the OFL, ABC, and ACT for the prev year (terminal year of previous SAFE)
#####################################################################################################
...
load(paste0(dir_R,"\\Sablefish_Data_",SA_curr_YR,".RData",sep=''))        # for data 
...
#############################################################################################################################################
# SSB Projected Density Distribution (3.44)
############################################################################################################################################
...
ggsave(paste0(dir_results , "//Fig. 3.44. Projected_SSB_Density.png", sep=''),width=8,height=5,dpi=300,plot=plot_proj_ssb)


#############################################################################################################################################
# Retrospective SSB (3.45)
############################################################################################################################################
...
#############################################################################################################################################
# Retrospective Recruitment (3.46)
############################################################################################################################################
...
#############################################################################################################################################
# All Model Historical Retrospective SSB (3.47)
############################################################################################################################################
...
#############################################################################################################################################
# Current Model Historical Retrospective SSB (3.48)
############################################################################################################################################
...
#############################################################################################################################################
# Incremental Data Addition R and SSB (3.50)
############################################################################################################################################
...
#############################################################################################################################################
# Index Sensitivity Analysis R and SSB (3.51)
############################################################################################################################################
...
#############################################################################################################################################
# Sensitivity Runs R and SSB (3.52)
############################################################################################################################################
...
#############################################################################################################################################
# Contribution of Recent Yearclass to SSB (3.53)
############################################################################################################################################
...
ggsave(paste0(dir_results , "//Fig. 3.53. Percent Contr to SSB by YC.png"),plot=ssb_cont_plot,width=8,height=8,dpi=400)


#############################################################################################################################################
# Recruitment, SSB, Catch (3.54)
############################################################################################################################################
...
ggsave(paste0(dir_results , "//Fig. 3.54. SSB, Recruitment, Catch.png"),plot=rec_ssb_catch_plot,width=8,height=5,dpi=300)
...
area_dep_out = total_dep_area %>% group_by(Year) %>% filter (Year >= curr_yr-3) %>% select(Year, fmp_area, pred_dep) %>%
...
catch_term_subarea_apport = catch_term_subarea_out %>% filter(year == curr_yr) %>% group_by(fmp_subarea) %>%
...
write.csv(proj_bio_catch, paste0(dir_tables,"//_Exec_Summ_Tables_Age_4_Bio_total.csv", sep=''))
...
age4_bio_AI_proj_yr1 <-  proj_bio_catch %>% select(year, bio) %>% filter(year==(curr_yr+1)) %>% mutate(age_4_bio = bio*AI_prop, region = "AI")
age4_bio_AI_proj_yr2 <-  proj_bio_catch %>% select(year, bio) %>% filter(year==(curr_yr+2)) %>% mutate(age_4_bio = bio*AI_prop, region = "AI")

age4_bio_BS_proj_yr1 <-  proj_bio_catch %>% select(year, bio) %>% filter(year==(curr_yr+1)) %>% mutate(age_4_bio = bio*BS_prop, region = "BS")
age4_bio_BS_proj_yr2 <-  proj_bio_catch %>% select(year, bio) %>% filter(year==(curr_yr+2)) %>% mutate(age_4_bio = bio*BS_prop, region = "BS")

age4_bio_GOA_proj_yr1 <-  proj_bio_catch %>% select(year, bio) %>% filter(year==(curr_yr+1)) %>% mutate(age_4_bio = bio*GOA_prop, region = "GOA")
age4_bio_GOA_proj_yr2 <-  proj_bio_catch %>% select(year, bio) %>% filter(year==(curr_yr+2)) %>% mutate(age_4_bio = bio*GOA_prop, region = "GOA")
...
proj_age4_bio = age4_bio_AI_proj_yr1 %>% full_join(age4_bio_AI_proj_yr2) %>% full_join(age4_bio_BS_proj_yr1) %>% full_join(age4_bio_BS_proj_yr2) %>%
             full_join(age4_bio_GOA_proj_yr1) %>% full_join(age4_bio_GOA_proj_yr2)
...
table_3.1_gear = catch_gear_historical  %>%
                     select(!gear) %>% full_join(non_fsh_catch %>% rename("Year" = "year", "catch" = "non_fsh_catch") %>%
...
# reads in 'management history.csv' which is just table 3.2 (as of 2023); need to provide input of OFL, ABC, ACT for previous year at start of code
...
man_hist = as.data.frame(read_csv(paste0(dir_tables_hist,"//management history.csv",sep=''), col_types = "d")) 
...
# This is calculated in the apportionment spreadsheet All proportions_changable ABC_2022.xlsx

LL_srvy_RPN = data.frame(year = as.numeric(row.names(sab_curr$obssrv3)), 
                     Value = as.numeric(unlist(sab_curr$obssrv3["obssrv3"])),
                     Survey = "NOAA Domestic LL Survey RPNs") 

LL_srvy_RPW = data.frame(year = as.numeric(row.names(sab_curr$obssrv1)), 
                     Value = as.numeric(unlist(sab_curr$obssrv1["obssrv1"])),
                     Survey = "NOAA Domestic LL Survey RPWs") 

COOP_srvy_RPN = data.frame(year = as.numeric(row.names(sab_curr$obssrv2)), 
                         Value = as.numeric(unlist(sab_curr$obssrv2["obssrv2"])),
                         Survey = "Japanese COOP LL Survey RPWs") 

COOP_srvy_RPW = data.frame(year = as.numeric(row.names(sab_curr$obssrv4)), 
                         Value = as.numeric(unlist(sab_curr$obssrv4["obssrv4"])),
                         Survey = "Japanese COOP LL Survey RPNs") 
...
JAP_cpue_srvy = data.frame(year = as.numeric(row.names(sab_curr$obssrv6)), 
                       Value = as.numeric(unlist(sab_curr$obssrv6["obssrv6"])),
                       Survey = "Japanese Fishery CPUE RPWs") 

table_3.5 = COOP_srvy_RPN %>% full_join(LL_srvy_RPN) %>% full_join(COOP_srvy_RPW) %>% full_join(LL_srvy_RPW) %>% full_join(trwl_srvy) %>% 
               full_join(JAP_cpue_srvy) %>% full_join(cpue_srvy) %>% pivot_wider(names_from = Survey, values_from = (Value)) %>% rename("Fish_CPUE" = "Fishery CPUE RPWs") %>%
               mutate(across(-c(year,Fish_CPUE), round,0),
                      across(-year, ~ format(., big.mark = ",", scientific = F, trim=TRUE)),
                      Fish_CPUE = round(as.numeric(Fish_CPUE),2)) %>% rename("Year" = "year", "Fishery CPUE RPWs" = "Fish_CPUE") %>% arrange(Year)
...
table_3.7 = comp_wts_curr %>% full_join(index_wts_curr) %>% full_join(comp_wts_prev %>% full_join(index_wts_prev)) %>% 
                ...
                relocate('source') %>% rename("Data Source" = "source")
...
table_3.10 = table_3.8 %>% select(Year, `Recruitment (millions) MLE`, `SSB (kt) MLE`, `Biomass (kt) MLE`) %>% filter(Year < curr_yr) %>%
                      ...
                      relocate(bio, .before = `Biomass (kt) MLE`) %>%
                      ...
                      ungroup() %>% mutate(across(-Year, ~round(.,digits = 2))) 
...
ggsave(paste0(dir_extra , "//ALT_Fig Resid Comparison Fixed Gear Fishery Age Comp.png"),plot=fish1_age_resid_comb_plot,width=12,height=10,dpi=300)
...
ggsave(paste0(dir_extra , "//ALT_Fig Resid Comparison Dom LL Survey Age Comp.png"),plot=srv1_age_comb_plot,width=12,height=10,dpi=300)
...
ggsave(paste0(dir_extra , "//ALT_Fig Resid Comparison Japanese LL Survey Age Comp.png"),plot=srv2_age_comb_plot,width=12,height=10,dpi=300)
...
ggsave(paste0(dir_extra , "//ALT_Fig Resid Comparison Dom LL Survey Length Comp Female.png"),plot=srv1_fem_len_comb_plot,width=12,height=10,dpi=300)
...
ggsave(paste0(dir_extra , "//ALT_Fig Resid Comparison Dom LL Survey Length Comp Male.png"),plot=srv1_male_len_comb_plot,width=12,height=10,dpi=300)
...
ggsave(paste0(dir_extra , "//ALT_Fig Resid Comparison Japanese LL Survey Length Comp Female.png"),plot=srv2_fem_len_comb_plot,width=12,height=10,dpi=300)
...
ggsave(paste0(dir_extra , "//ALT_Fig Resid Comparison Japanese LL Survey Length Comp Male.png"),plot=srv2_male_len_comb_plot,width=12,height=10,dpi=300)
...
ggsave(paste0(dir_extra , "//ALT_Fig Resid Comparison Fixed Gear Fishery Length Comp Female.png"),plot=fish1_fem_len_comb_plot,width=12,height=10,dpi=300)
...
ggsave(paste0(dir_extra , "//ALT_Fig Resid Comparison Fixed Gear Fishery Length Comp Male.png"),plot=fish1_male_len_comb_plot,width=12,height=10,dpi=300)
...
ggsave(paste0(dir_extra , "//ALT_Fig Resid Comparison Trawl Fishery Length Comp Female.png"),plot=fish3_fem_len_comb_plot,width=12,height=10,dpi=300)
...
ggsave(paste0(dir_extra , "//ALT_Fig Resid Comparison Trawl Fishery Length Comp Male.png"),plot=fish3_male_len_comb_plot,width=12,height=10,dpi=300)
...
ggsave(paste0(dir_extra , "//ALT_Fig Resid Comparison Trawl Survey Length Comp Female.png"),plot=srv7_fem_len_comb_plot,width=12,height=10,dpi=300)
...
ggsave(paste0(dir_extra , "//ALT_Fig Resid Comparison Trawl Survey Length Comp Male.png"),plot=srv7_male_len_comb_plot,width=12,height=10,dpi=300)
...
######################################################################################################################################################################  
  save.image(file=paste0(dir_extra,"//Sablefish_Results_",year,".RData",sep=''))        # Save all the data frames in case want to reload data without doing full data pulls
######################################################################################################################################################################
