...
# Weight at age
data$WAA <- array(NA, dim = c(data$n_regions, length(data$yrs), length(data$ages), data$n_sexes)) # weight-at-age array
...
# Maturity at age
data$MatAA <- array(0,  dim = c(data$n_regions, length(data$yrs), length(data$ages), data$n_sexes)) # maturity at age
...
data$SizeAgeTrans <- array(0, dim = c(data$n_regions, length(data$yrs), length(data$lens), length(data$ages), data$n_sexes)) # size age transition matrix
...
colnames(data$ObsFishIdx) <- data$yrs # define row years
colnames(data$ObsFishIdx_SE) <- data$yrs # define row years
...
data$ObsFishAgeComps <- array(NA, dim = c(data$n_regions, length(data$yrs), length(data$ages), data$n_sexes, data$n_fish_fleets))
colnames(data$ObsFishAgeComps) <- data$yrs # define row years
...
data$UseFishAgeComps <- array(0, dim = c(data$n_regions, length(data$yrs), data$n_fish_fleets))
colnames(data$UseFishAgeComps) <- data$yrs # define row years
...
data$ISS_FishAgeComps <- array(0, dim = c(data$n_regions, length(data$yrs), data$n_sexes, data$n_fish_fleets))
colnames(data$ISS_FishAgeComps) <- data$yrs # define row years
...
data$ObsFishLenComps <- array(NA, dim = c(data$n_regions, length(data$yrs), length(data$lens), data$n_sexes, data$n_fish_fleets))
colnames(data$ObsFishLenComps) <- data$yrs # define row years
...
data$UseFishLenComps <- array(0, dim = c(data$n_regions, length(data$yrs), data$n_fish_fleets))
colnames(data$UseFishLenComps) <- data$yrs # define row years
...
data$ISS_FishLenComps <- array(0, dim = c(data$n_regions, length(data$yrs), data$n_sexes, data$n_fish_fleets))
colnames(data$ISS_FishLenComps) <- data$yrs # define row years
...
data$FishAgeComps_LikeType <- array(0, dim = c(data$n_regions, data$n_fish_fleets)) # multinomial for both fleets 
data$FishLenComps_LikeType <- array(0, dim = c(data$n_regions, data$n_fish_fleets)) # multinomial for both fleets 
data$FishAgeComps_Type <- array(2, dim = c(data$n_regions, data$n_fish_fleets)) # Joint age comps
data$FishLenComps_Type <- array(2, dim = c(data$n_regions, data$n_fish_fleets)) # Joint length comps
...
colnames(data$ObsSrvIdx) <- data$yrs # define row years
colnames(data$ObsSrvIdx_SE) <- data$yrs # define row years
...
# Survey Age Comps
data$ObsSrvAgeComps <- array(NA, dim = c(data$n_regions, length(data$yrs), length(data$ages), data$n_sexes, data$n_srv_fleets))
colnames(data$ObsSrvAgeComps) <- data$yrs # define row years
...
colnames(data$UseSrvAgeComps) <- data$yrs # define row years
...
# Data weighting for fishery age compositions
data$ISS_SrvAgeComps <- array(0, dim = c(data$n_regions, length(data$yrs), data$n_sexes, data$n_srv_fleets))
colnames(data$ISS_SrvAgeComps) <- data$yrs # define row years
...
# Survey Length Comps
data$ObsSrvLenComps <- array(NA, dim = c(data$n_regions,length(data$yrs), length(data$lens), data$n_sexes, data$n_srv_fleets))
colnames(data$ObsSrvLenComps) <- data$yrs # define row years
...
data$UseSrvLenComps <- array(0, dim = c(data$n_regions, length(data$yrs), data$n_srv_fleets))
colnames(data$UseSrvLenComps) <- data$yrs # define row years
...
data$ISS_SrvLenComps <- array(0, dim = c(data$n_regions,length(data$yrs), data$n_sexes, data$n_srv_fleets))
colnames(data$ISS_SrvLenComps) <- data$yrs # define row years
...
# Composition munging stuff
data$SrvAgeComps_LikeType <- array(0, dim = c(data$n_regions, data$n_srv_fleets)) # multinomial for both survey fleet 
data$SrvLenComps_LikeType <- array(0, dim = c(data$n_regions, data$n_srv_fleets)) # multinomial for both survey fleet 
data$SrvLenComps_Type <- array(1, dim = c(data$n_regions, data$n_srv_fleets)) # split for both survey fleet length
data$SrvAgeComps_Type <- array(2, dim = c(data$n_regions, data$n_srv_fleets)) # split for both survey age length
...
# Fishing Mortality
parameters$ln_obscatch_sigma <- array(0.1, dim = c(data$n_regions, data$n_fish_fleets))
...
parameters$ln_F_devs <- array(0, dim = c(data$n_regions, length(data$yrs), data$n_fish_fleets)) # fishing mortality deviations from mean
...
parameters$ln_fishsel_dev1 <- array(0, dim = c(data$n_regions, length(data$yrs), data$n_sexes, data$n_fish_fleets))
parameters$ln_fishsel_dev2 <- array(0, dim = c(data$n_regions, length(data$yrs), data$n_sexes, data$n_fish_fleets))
...
# Recruitment -------------------------------------------------------------
parameters$ln_R0 <- array(tem_par$coefficients[names(tem_par$coefficients) == "log_mean_rec"], dim = c(data$n_regions)) # mean recruitment
...
parameters$ln_RecDevs <- array(tem_par$coefficients[str_detect(names(tem_par$coefficients), "rec_dev")][c(29:89)], 
                               dim = c(data$n_regions, length(data$yrs) - 1))
...
mapping$ln_obscatch_sigma <- factor(rep(NA, length(parameters$ln_obscatch_sigma)))
mapping$ln_sigmaF <- factor(rep(NA, length(parameters$ln_sigmaF)))
