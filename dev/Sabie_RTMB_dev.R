...
data$WAA[,,1] <- matrix(waa_f, nrow = length(1:62), ncol = length(1:30), byrow = TRUE) # female weight-at-age
data$WAA[,,2] <- matrix(waa_m, nrow = length(1:62), ncol = length(1:30), byrow = TRUE) # male weight-at-age

# Maturity at age
data$MatAA <- array(0,  dim = c(length(data$yrs), length(data$ages), data$n_sexes)) # maturity at age
data$MatAA[1:62,,1] <- aperm(replicate(length(1:62), mat), c(2,1))
data$MatAA[1:62,,2] <- aperm(replicate(length(1:62), mat), c(2,1))
...
data$SizeAgeTrans[36:62,,,1] <- aperm(replicate(length(36:62), tem_dat$sizeage.f.block2), perm = c(3,2,1)) # female
data$SizeAgeTrans[36:62,,,2] <- aperm(replicate(length(36:62), tem_dat$sizeage.m.block2), perm = c(3,2,1)) # male
...
data$ObsCatch <- matrix(NA, nrow = length(1:62), ncol = data$n_fish_fleets)
rownames(data$ObsCatch) <- data$yrs # define row years
data$ObsCatch[,1] <- as.numeric(strsplit(tem_admb_dat[46], split = " ")[[1]])[1:62] # fixed gear catches
data$ObsCatch[,2] <- as.numeric(strsplit(tem_admb_dat[48], split = " ")[[1]])[1:62] # trawl gear catches
data$ObsCatch[data$ObsCatch == 0] <- NA # set 0 catches to NA so we aren't fitting
data$UseCatch <- matrix(1, nrow = length(1:62), ncol = data$n_fish_fleets)
...
# Fishery Indices
data$ObsFishIdx <- matrix(NA, nrow = length(1:62), ncol = data$n_fish_fleets)
data$ObsFishIdx_SE <- matrix(NA, nrow = length(1:62), ncol = data$n_fish_fleets)
data$UseFishIdx <- matrix(1, nrow = length(1:62), ncol = data$n_fish_fleets)
...
data$ObsFishIdx[rownames(data$ObsFishIdx) %in% rownames(tem_dat$obssrv5)[1:27],1] <- as.numeric(strsplit(tem_admb_dat[127], split = " ")[[1]])[1:27]
data$ObsFishIdx_SE[rownames(data$ObsFishIdx_SE) %in% rownames(tem_dat$obssrv5)[1:27],1] <- as.numeric(strsplit(tem_admb_dat[129], split = " ")[[1]])[1:27]
...
data$ObsFishAgeComps[rownames(data$ObsFishAgeComps) %in% rownames(compdata_2021$FishAge),,,] <- compdata_2021$FishAge # Observed fixed gear fishery age comps (sex specific)
data$UseFishAgeComps <- matrix(0, nrow = length(data$yrs), ncol = data$n_fish_fleets)
rownames(data$UseFishAgeComps) <- data$yrs # define row years
data$UseFishAgeComps[which(rowSums(compdata_2021$FishAge) != 0),1] <- 1 # only fit if have age comp data
...
# observed fixed gear fishery length comps (split)
...
# observed trawl gear fishery length comps (split)
...
data$ObsFishLenComps[rownames(data$ObsFishLenComps) %in% rownames(compdata_2021$FishLen),,,] <- compdata_2021$FishLen
...
data$UseFishLenComps[which(rowSums(compdata_2021$FishLen[,,,1]) != 0),1] <- 1 # only fit if have length comp data (fixed gear)
data$UseFishLenComps[which(rowSums(compdata_2021$FishLen[,,,2]) != 0),2] <- 1 # only fit if have length comp data (trawl gear)
...
# joint iss
data$ISS_FishLenComps[which(rowSums(compdata_2021$FishLen[,,,1]) != 0),1,1] <- 20 # Assuming constant ISS of 20 for fixed gear fishery
data$ISS_FishLenComps[which(rowSums(compdata_2021$FishLen[,,,2]) != 0),1,2] <- 20 # Assuming constant ISS of 20 for trawl gear fishery
data$Wt_FishLenComps <- array(0, dim = c(data$n_sexes, data$n_fish_fleets)) # weights for fishery age comps
data$Wt_FishLenComps[1,1] <- 2.099563 # Weight for fixed gear len comps females
data$Wt_FishLenComps[1,2] <- 0.6589962 # Weight for trawl gear len comps females

# Composition munging stuff
data$FishAgeComps_LikeType <- rep(0, data$n_fish_fleets) # multinomial for both fleets 
data$FishLenComps_LikeType <- rep(0, data$n_fish_fleets) # multinomial for both fleets 
# data$FishAgeComps_Type <- rep(0,data$n_fish_fleets) # Aggregated age comps
# data$FishLenComps_Type <- rep(1,data$n_fish_fleets) # Split length comps
data$FishAgeComps_Type <- rep(2, data$n_fish_fleets) # Joint age comps
data$FishLenComps_Type <- rep(2, data$n_fish_fleets) # Joint length comps
...
# Survey Indices
data$ObsSrvIdx <- matrix(NA, nrow = length(1:62), ncol = data$n_srv_fleets)
data$ObsSrvIdx_SE <- matrix(NA, nrow = length(1:62), ncol = data$n_srv_fleets)
data$UseSrvIdx <- matrix(1, nrow = length(1:62), ncol = data$n_srv_fleets)
...
data$ObsSrvIdx[rownames(data$ObsSrvIdx) %in% rownames(tem_dat$obssrv3)[1:32],1] <- as.numeric(strsplit(tem_admb_dat[93], split = " ")[[1]])[1:32]
data$ObsSrvIdx_SE[rownames(data$ObsSrvIdx_SE) %in% rownames(tem_dat$obssrv3)[1:32],1] <- as.numeric(strsplit(tem_admb_dat[95], split = " ")[[1]])[1:32]

# Domestic Trawl Survey
data$ObsSrvIdx[rownames(data$ObsSrvIdx) %in% rownames(tem_dat$obssrv7)[1:14],2] <- as.numeric(strsplit(tem_admb_dat[161], split = " ")[[1]])[1:14]
data$ObsSrvIdx_SE[rownames(data$ObsFishIdx_SE) %in% rownames(tem_dat$obssrv7)[1:14],2] <- as.numeric(strsplit(tem_admb_dat[163], split = " ")[[1]])[1:14]
...
# joint ages
data$ObsSrvAgeComps[rownames(data$ObsSrvAgeComps) %in% rownames(compdata_2021$SrvAge),,,] <- compdata_2021$SrvAge
...
data$UseSrvAgeComps[which(rowSums(compdata_2021$SrvAge[,,,1]) != 0),1] <- 1 
data$UseSrvAgeComps[which(rowSums(compdata_2021$SrvAge[,,,3]) != 0),3] <- 1 
...
data$ISS_SrvAgeComps[which(rowSums(compdata_2021$SrvAge[,,,1]) != 0),1,1] <- 20 # Assuming constant ISS of 20 for domestic ll survey ages (aggregated)
data$ISS_SrvAgeComps[which(rowSums(compdata_2021$SrvAge[,,,3]) != 0),1,3] <- 20 # Assuming constant ISS of 20 for coop jp survey ages (aggregated)

data$Wt_SrvAgeComps <- array(NA, dim = c(data$n_sexes, data$n_srv_fleets)) # weights for survey age comps
data$Wt_SrvAgeComps[1,1] <- 5.399427 # Weight for domestic survey ll gear age comps
data$Wt_SrvAgeComps[1,3] <- 3.240157 # Weight for coop jp survey ll gear age comps
...
data$ObsSrvLenComps[rownames(data$ObsSrvLenComps) %in% rownames(tem_dat$olc.srv1.f)[1:32],,1,1] <- tem_dat$olc.srv1.f[1:32,] # females
data$ObsSrvLenComps[rownames(data$ObsSrvLenComps) %in% rownames(tem_dat$olc.srv1.m)[1:32],,2,1] <- tem_dat$olc.srv1.m[1:32,] # males

# observed domestic trawl survey length comps
data$ObsSrvLenComps[rownames(data$ObsSrvLenComps) %in% rownames(tem_dat$olc.srv7.f)[1:14],,1,2] <- tem_dat$olc.srv7.f[1:14,] # females
data$ObsSrvLenComps[rownames(data$ObsSrvLenComps) %in% rownames(tem_dat$olc.srv7.m)[1:14],,2,2] <- tem_dat$olc.srv7.m[1:14,] # males
...
data$UseSrvLenComps[rownames(data$UseSrvLenComps) %in% rownames(tem_dat$olc.srv1.f)[1:32],1] <- 0 # only fit if have len comp data
data$UseSrvLenComps[rownames(data$UseSrvLenComps) %in% rownames(tem_dat$olc.srv7.f)[1:14],2] <- 1 # only fit if have len comp data
data$UseSrvLenComps[rownames(data$UseSrvLenComps) %in% rownames(tem_dat$olc.srv2.f),3] <- 0 # only fit if have len comp data
...
data$ISS_SrvLenComps[rownames(data$ISS_SrvLenComps) %in% rownames(tem_dat$olc.srv1.f)[1:32],1,1] <- 20 # Assuming constant ISS of 20 for domestic ll survey females
data$ISS_SrvLenComps[rownames(data$ISS_SrvLenComps) %in% rownames(tem_dat$olc.srv1.m)[1:32],2,1] <- 20 # Assuming constant ISS of 20 for domestic ll survey males
data$ISS_SrvLenComps[rownames(data$ISS_SrvLenComps) %in% rownames(tem_dat$olc.srv7.f)[1:14],1,2] <- srv_trawl_iss[1:14] # Assuming constant ISS of 20 for domestic trawl survey females
data$ISS_SrvLenComps[rownames(data$ISS_SrvLenComps) %in% rownames(tem_dat$olc.srv7.m)[1:14],2,2] <- srv_trawl_iss[1:14] # Assuming constant ISS of 20 for domestic trawl survey males
...
# data$Wt_SrvLenComps[1,1] <- 1.49981566217443 # Weight for domestic ll survey len comps females
...
data$Wt_SrvLenComps[1,1] <- 1 # Weight for domestic ll survey len comps females
data$Wt_SrvLenComps[2,1] <- 1 # Weight for domestic ll survey len comps males
data$Wt_SrvLenComps[1,2] <- 0.5916596  # Weight for domestic trawl survey len comps females
data$Wt_SrvLenComps[2,2] <- 0.3986298  # Weight for domestic trawl survey len comps males
data$Wt_SrvLenComps[1,3] <- 1 # Weight for coop jp ll survey len comps females
data$Wt_SrvLenComps[2,3] <- 1  # Weight for coop jp ll survey len comps males

# Composition munging stuff
data$SrvAgeComps_LikeType <- rep(0, data$n_srv_fleets) # multinomial for both survey fleet 
data$SrvLenComps_LikeType <- rep(0, data$n_srv_fleets) # multinomial for both survey fleet 
# data$SrvAgeComps_Type <- rep(0,data$n_srv_fleets) # Aggregated age comps
data$SrvLenComps_Type <- rep(1, data$n_srv_fleets) # split length comps
data$SrvAgeComps_Type <- rep(2, data$n_srv_fleets) # Split age comps

### Fishery Stuff -----------------------------------------------------
# Selectivity
data$cont_tv_fish_sel <- c(0, 0) # no timevarying selex continously
# Time Block Specification
data$fish_sel_blocks <- matrix(NA, nrow = length(1:62), ncol = data$n_fish_fleets)
data$fish_sel_blocks[1:35,1] <- 0 # block one fishery ll selex
data$fish_sel_blocks[36:56,1] <- 1 # block two fishery ll selex
data$fish_sel_blocks[57:62,1] <- 2 # block three fishery ll selex
...
### Survey Selectivity ------------------------------------------------------
# Time Block Specification
data$srv_sel_blocks <- matrix(NA, nrow = length(1:62), ncol = data$n_srv_fleets)
data$srv_sel_blocks[1:56,1] <- 0 # block one survey ll selex
data$srv_sel_blocks[57:62,1] <- 1 # block two survey ll selex
...
### Survey Catchability ----------------------------------------------------
# Time Block Specification
data$srv_q_blocks <- matrix(0, nrow = length(1:62), ncol = data$n_srv_fleets) # catchability blocks are same as selectivity blocks
...
parameters$ln_F_devs[,1] <- devs_ll_fish[1:62] # longline fishery Fs
parameters$ln_F_devs[,2] <- c(rep(0, 3), devs_ll_trwl[1:59] ) # trwl fishery Fs
...
parameters$ln_fish_fixed_sel_pars[,1,1,2] <- c(1.7735e+00, -1) # Female trawl fishery first time block parameters (amax and then power)
parameters$ln_fish_fixed_sel_pars[,2,1,2] <- c(1.7735e+00, -1) # Female trawl fishery first time block parameters (amax and then power)
parameters$ln_fish_fixed_sel_pars[,3,1,2] <- c(1.7735e+00, -1) # Female trawl fishery first time block parameters (amax and then power)

# Trawl Fishery kept the same across blocks
parameters$ln_fish_fixed_sel_pars[,1,2,2] <- c(2.1060e+00, -1) # Male trawl fishery first time block parameters (amax and then power)
parameters$ln_fish_fixed_sel_pars[,2,2,2] <- c(2.1060e+00, -1) # Male trawl fishery first time block parameters (amax and then power)
parameters$ln_fish_fixed_sel_pars[,3,2,2] <- c(2.1060e+00, -1) # Male trawl fishery first time block parameters (amax and then power)
...
parameters$ln_RecDevs <- tem_par$coefficients[str_detect(names(tem_par$coefficients), "rec_dev")][c(29:89)]
...
parameters$ln_FishAge_DM_theta <- rep(0, data$n_fish_fleets)
parameters$ln_FishLen_DM_theta <- rep(0, data$n_fish_fleets)
parameters$ln_SrvAge_DM_theta <- rep(0, data$n_srv_fleets)
parameters$ln_SrvLen_DM_theta <- rep(0, data$n_srv_fleets)
...
mapping$ln_F_devs <- factor(c(1:length(data$yrs), rep(NA, 3), 65:(126-3))) # fix all fishery mort parameters for now
...
par_df <- rbind(M_df, R0_df, srv_q_df, fish_q_df)
...
rec_se_series <- data.frame(Par = "Recruitment (SE)",
                            ...
                            ADMB = tem_par$se[str_detect(names(tem_par$se), "pred_rec")][1:62])
...
females_series <- data.frame(Par = "Total Females",
                             Year = 1960:2021,
                             TMB = rowSums(sabie_rtmb_model$rep$NAA[-63,,1]),
                             ADMB = tem_dat$t.series$numbers.f[1:62])

males_series <- data.frame(Par = "Total Males",
                           Year = 1960:2021,
                           TMB = rowSums(sabie_rtmb_model$rep$NAA[-63,,2]),
